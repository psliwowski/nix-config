rec {
  ...
  mkHomeDir =
    ...
    in
    if (cfg.host.homeDirectory or null) != null then
      cfg.host.homeDirectory
  ...
  mkCfg =
    ...
    in
    assert
      builtins.isAttrs host || abort "mkCfg: host must be an attribute set with username and system";
    assert
      isNonEmptyStr (host.username or null) || abort "mkCfg: host.username must be a non-empty string.";
    assert
      isNonEmptyStr (host.system or null)
      || abort "mkCfg: host.system must be a non-empty string (e.g. 'aarch64-darwin' or 'x86_64-linux')";
    assert
      homeDirectory == null
      || isNonEmptyStr homeDirectory
      || abort "mkCfg: host.homeDirectory must be a non-empty string if provided.";
    assert builtins.isList modules || abort "mkCfg: modules must be a list of module names";
    assert
      (builtins.isAttrs configuration || builtins.isFunction configuration)
